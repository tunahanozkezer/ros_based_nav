...
void process_image_callback(const sensor_msgs::Image img)
{
	...
	if (white_location >= 0){
		...
		const int horizontal_location = white_location % img.step;
		// Consider leftmost 5/12 of the image as left and rightmost 5/12 of it as right
		// Center 1/6 of it considered as center
		const float left_threshold = img.step / 3;
		const float right_threshold = 2 * img.step / 3;
...
int main(int argc, char** argv)
{
    ...
    const ros::Subscriber sub1{n.subscribe("/camera/rgb/image_raw", 10, process_image_callback)};
